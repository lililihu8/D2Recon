...
class DrivingDataset(SceneDataset):
    def __init__(
        ...
        #   PandaSet: 6 Cameras
        #   NuPlan:   8 Cameras
        if self.data_cfg.mode is not None:
            self.mode = self.data_cfg.mode
        else:
            self.mode = "eval" 
        ...
        # ---- create data source ---- #
        self.pixel_source, self.lidar_source = self.build_data_source(mode=self.mode)
        if self.mode == "train":
    ...
    def build_data_source(self, mode: str):
        """
        ...
        if self.data_cfg.lidar_source.load_lidar and mode == "train":
    ...
    def get_lidar_samples(
        ...
        if return_indices:
            sampled_indices = sampled_idx.to(device)
        
        return sampled_pts, sampled_color, sampled_time, sampled_indices
    ...
    def filter_pts_in_boxes(
        ...
        seed_lidar_indices: Tensor = None,
    ):
        """
        ...
        if seed_lidar_indices is not None:
            seed_lidar_indices = seed_lidar_indices[~inside_mask]
        ...
        return {
            ...
            "colors": seed_colors,
            "time": seed_time,
            "lidar_indices": seed_lidar_indices,
        }

    def classify_background_points(
        self,
        seed_pts: Tensor,
        seed_colors: Tensor = None,
        lidar_indices: Tensor = None,
        tree_height_thresh: float = 2.5,
        tree_tall_thresh: float = 4.0,
        tree_green_thresh: float = -0.02,
    ) -> Dict[str, Dict[str, Tensor]]:
        num_points = seed_pts.shape[0]
        device = seed_pts.device
        ground_mask = torch.zeros(num_points, dtype=torch.bool, device=device)

        if lidar_indices is not None and hasattr(self.lidar_source, "grounds"):
            valid_indices = lidar_indices >= 0
            if valid_indices.any():
                ground_mask[valid_indices] = self.lidar_source.grounds[lidar_indices[valid_indices].long()].to(device)

        if ground_mask.any():
            ground_floor = torch.quantile(seed_pts[ground_mask, 2], 0.05)
        else:
            ground_floor = torch.quantile(seed_pts[:, 2], 0.1)
            ground_mask = seed_pts[:, 2] <= (ground_floor + 0.35)

        relative_height = seed_pts[:, 2] - ground_floor
        tree_mask = (~ground_mask) & (relative_height > tree_height_thresh)
        if seed_colors is not None:
            green_score = seed_colors[:, 1] - 0.5 * (seed_colors[:, 0] + seed_colors[:, 2])
            tree_mask = tree_mask & ((green_score > tree_green_thresh) | (relative_height > tree_tall_thresh))
        else:
            tree_mask = tree_mask & (relative_height > tree_tall_thresh)
        other_mask = ~(ground_mask | tree_mask)
        ...
        grouped_points = {}
        for group_name, mask in grouped_masks.items():
            grouped_points[group_name] = {
                "pts": seed_pts[mask],
                "colors": None if seed_colors is None else seed_colors[mask],
                "mask": mask,
            }
    ...
    def project_lidar_pts_on_images(self, delete_out_of_view_points=True):
        ...
        for cam in self.pixel_source.camera_data.values():
            ...
            for frame_idx in tqdm(
                ...
                # depth_path  = os.path.join(self.data_path, "depth_maps")
                # os.makedirs(depth_path, exist_ok=True)
                # depth_map_save_path = os.path.join(depth_path, f"{frame_idx:03d}_{cam.cam_id}")
                # depth_map = depth_map.cpu().numpy()
                # np.save(depth_map_save_path, depth_map)
