...
def get_filenames(segment_name, opt):
    ...
    """
    if opt.dataset == 'render':
        rgb_dir_path = osp.join(opt.data_path, segment_name, 'videos', 'full_set_30000', 'rgbs')
        frame_indices = sorted([
            int(osp.splitext(f)[0].split('_')[0])
            for f in os.listdir(rgb_dir_path)
            if osp.splitext(f)[1] == opt.eval_img_ext
        ])
...
def is_edge(filename, opt):
    ...
    if opt.dataset == 'render':
        ...
        left_bound = osp.join(opt.data_path, seg_name, 'videos', 'full_set_30000', 'rgbs', f'{left_index:03d}_000{opt.eval_img_ext}')
        right_bound = osp.join(opt.data_path, seg_name, 'videos', 'full_set_30000', 'rgbs', f'{right_index:03d}_000{opt.eval_img_ext}')
