...
def vis_segment(opt, trainer, val_segment, outdir):
    ...
    segment_name = val_segment.split('/')[1]
    out_vid_name = osp.join(outdir, '{}.mp4'.format(segment_name))
    frame_dir = osp.join(outdir, '{}_frames'.format(segment_name))
    os.makedirs(frame_dir, exist_ok=True)

    for idx, frame in enumerate(out_frames):
        Image.fromarray(frame).save(osp.join(frame_dir, f"{idx:06d}.png"))

    fps = 13 if opt.dataset == "nuscenes" else 10   # dataset info 
    make_mp4(out_frames, out_vid_name, fps=fps, bgr=False)
    print(f'Saved video to `{out_vid_name}`')
