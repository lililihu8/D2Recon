...
def main():
    ...
    if opt.dataset == 'kitti':
        ...
        if len(filenames) == 0:
            out_strings.append('Mask Split Evaluation Skipped: split file is empty.')
            filenames = None
        if filenames is not None:
            ...
            if not getattr(dataset, 'has_mask_gt', True):
                out_strings.append(f'Mask Split Evaluation Skipped for {opt.dataset}: no segmentation annotations.')
            else:
                loader = DataLoader(dataset, opt.batch_size, False, num_workers=opt.num_workers, pin_memory=True, drop_last=False)
                out_strings.append(f'=== len={len(dataset)} ===')
                out_strings.append(header_str)
