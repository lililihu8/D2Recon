...
def main():
    ...
    if len(filenames) == 0:
        print(f'Skipping motion segmentation for {opt.dataset}: test_mask_files.txt is empty.')
        return
    ...
    if not getattr(dataset, 'has_mask_gt', True):
        print(f'Skipping motion segmentation for {opt.dataset}: no segmentation annotations are available.')
        return
