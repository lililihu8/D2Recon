...
def eval_odom(opt, trainer, val_segment, track_length):
    ...
    if opt.dataset == 'render':
        gt_global_poses = dataset.get_odometry(val_segment)
    else:
        gt_poses_path = osp.join(opt.data_path, val_segment, opt.cam_name, 'odometry.txt')
        gt_global_poses = np.loadtxt(gt_poses_path).reshape(-1, 4, 4)
    ...
    for i in range(0, num_frames - 1):
        ...
        if len(local_xyzs) != len(gt_local_xyzs): continue
...
def main():
    ...
    opt.num_workers = 1
    opt.batch_size = 1
    assert opt.dataset in ['waymo', 'nuscenes', 'render'], f'Only implemented for waymo, nuscenes, and render, {opt.dataset} is not supported.'
