...
class DepthMetrics(nn.Module):
    ...
    def forward(self, inputs, outputs, mask=None):
        ...
        depth_valid = inputs['depth_valid']    
        ...
        for bi, (disp_p, depth_g, valid, dim) in enumerate(zip(disp_pred, depth_gt, depth_valid, gt_dim)):
            ...
            # if depth_g.ndim == 2:
            #     crop_valid = torch.zeros_like(depth_g, dtype=torch.bool)
            #     crop_valid[up:down, left:right] = True
            #     valid = valid.bool()
            #     valid = torch_and(valid,
            #                       crop_valid,
            #                       depth_g > self.min_depth,
            #                       depth_g < self.max_depth)
            #     d_gt = depth_g[valid]
            #     d_pd = depth_p[valid]
            #     valid_ind = valid
            # else:
