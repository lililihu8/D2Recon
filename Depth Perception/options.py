...
class DynamoOptions:
	def __init__(self):
		...
		self.p.add_argument("--dataset", "-d",
							...
							choices=["kitti", "waymo", "nuscenes", "render"],
							default="waymo")
		...
		self.p.add_argument("--train_img_type",
							...
							choices=["original", "downsample", "rgbs"],
							default=None)
	...
	def parse(self, **kwargs):
		...
		dataset_conf = {
			'split': 			{"waymo": "waymo", "nuscenes": "nuscenes", "kitti": "eigen_zhou", "render": "render"},		# train/test split name
			'height': 			{"waymo": 320, "nuscenes": 288, "kitti": 192, "render": 320},							# height used for training
			'width': 			{"waymo": 480, "nuscenes": 512, "kitti": 640, "render": 480},							# width used for training
			'cam_name': 		{"waymo": 'FRONT', "nuscenes": 'FRONT', "kitti": 'image_02', "render": 'FRONT'},			# only used for eval in kitti 
			'train_img_type': 	{"waymo": 'downsample', "nuscenes": 'downsample', "kitti": 'downsample', "render": 'rgbs'},#image type used for training
			'eval_max_depth': 	{"waymo": 75, "nuscenes": 75, "kitti": 80, "render": 75},								# max depth bound used for evaluation
			'eval_img_bound': 	{"waymo": 	  [0, 1, 0, 1], 											# image bound used for evaluation 
								 "nuscenes":  [0, 1, 0, 1], 
								 "kitti": 	  [0.40810811, 0.99189189, 0.03594771,  0.96405229],
								 "render":   [0, 1, 0, 1]},		# https://github.com/nianticlabs/monodepth2/blob/b676244e5a1ca55564eb5d16ab521a48f823af31/evaluate_depth.py#L193
			'eval_img_ext': 	{"waymo": '.jpg', "nuscenes": '.jpg', "kitti": '.png', "render": '.jpg'},					
			'eval_img_type': 	{"waymo": 'downsample', "nuscenes": 'downsample', "kitti": 'original', "render": 'rgbs'},	
		}
